...
import org.apache.spark.ml.utils.Utils.timeIt
...
/**
  ...
  *      {k java.util.Random java.util.Random}, partly because of the algorithm, but also due
...
object XORShiftRandom {
  ...
  def benchmark(numIters: Int): Map[String, Long] = {
    ...
    Map("javaTime" -> timeIt(numIters) {
      javaRand.nextInt()
    },
      "xorTime" -> timeIt(numIters) {
        xorRand.nextInt()
      })
  }
}
